k = `rtZJTZSxqYbV8YwcgJIYQ1AdS8pZMqMD`;
...
function handleGetHolidays(evt){
    ...
    let leapYear = false;
    if ((0 == userYear.value % 4) && (userYear.value % 100 !== 0) || (userYear.value % 400 === 0)){
        leapYear = true;
    }
    ...
    if (userCountry.value === 'Please Choose a Country') {
        ...
        $.ajax({
            ...
            error: function(error) {
                ...
                // Display specific error message based on error status code (optional)
            },
            complete: function() {
